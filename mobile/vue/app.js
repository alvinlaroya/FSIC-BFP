...
var app = new Vue({
    ...
    data: {
        ...
        fburls: [
            'https://m.facebook.com/story.php?story_fbid=3797130953694960&id=100001942779227',
            'https://m.facebook.com/story.php?story_fbid=3797130953694960&id=100001942779227',
            'https://m.facebook.com/story.php?story_fbid=3797130953694960&id=100001942779227',
        ],
        alertoverlay: false,
    ...
    methods: {
        ...
        previewImage(event) {
           ...
           /*  $('#alertNow').attr('src', event.target.files[0]);
        ...
        onUpload(){
            ...
            storageRef.on('state_changed', snapshot => {
                ...
                storageRef.snapshot.ref.getDownloadURL().then((url) => {
                    ...
                    var alertFire = {
                        id: counter,
                        witness_id: $("#session_id").val(),
                        witness_pic: $("#session_picture").val(),
                        witness_name: $("#session_name").val(),
        ...
        getSessionInfo(){
            this.alert.witness_id = $("#session_id").val();
            this.alert.witness_pic = $("#session_picture").val();
            this.alert.witness_name = $("#session_name").val();

            this.mobalertSession.name = $("#session_name").val();
            this.mobalertSession.email = $("#session_email").val();
            this.mobalertSession.picture = $("#session_picture").val();
            console.log("SESSION: " +document.getElementById("session_id").value)
        },
        addFsic() {
            var today = new Date();
            var time = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds() + ' ' + today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
            var fsic_sessionId = document.getElementById("session_id").value;
            var fsic_sessionPicture = document.getElementById("session_picture").value;
            alert(fsic_sessionPicture)
            let dateId = new Date();
            let timeId = dateId.getTime();
            let counter = timeId;
            var fsicFire = {
                id: counter,
                applicantId: fsic_sessionId,
                applicantPicture: fsic_sessionPicture,
                name: this.fsic.name,
        ...
        showInccident(alert){
            ...
            this.navigateToWaze = 'waze://https://waze.com/ul?ll='+alert.witness_lat+','+alert.witness_long+'&navigate=yes'
